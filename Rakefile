...
task :gem do
  %x{gem build acl_permalink.gemspec}
end
...
task :guide do
  ...
  buffer << read_comments("lib/acl_permalink.rb")
  buffer << read_comments("lib/acl_permalink/base.rb")
...
namespace :db do
  ...
  task :create => :load_path do
    require "helper"
    driver = AclPermalink::Test::Database.driver
    config = AclPermalink::Test::Database.config[driver]
    commands = {
