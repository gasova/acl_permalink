  ...
  module History
    ...
    def self.included(model_class)
      model_class.instance_eval do
        raise "AclPermalink::History is incompatible with AclPermalink::Scoped" if self < Scoped
        @acl_permalink_config.use :slugged
        has_many :slugs, :as => :sluggable, :dependent => :destroy,
          :class_name => Slug.to_s, :order => "#{Slug.quoted_table_name}.id DESC"
        after_save :create_slug
        relation_class.send :include, FinderMethods
        acl_permalink_config.slug_generator_class.send :include, SlugGenerator
      end
    end
    ...
    def create_slug
      return unless acl_permalink
      return if slugs.first.try(:slug) == acl_permalink
      # Allow reversion back to a previously used slug
      relation = slugs.where(:slug => acl_permalink)
      result = relation.select("id").lock(true).all
      relation.delete_all unless result.empty?
      slugs.create! do |record|
        record.slug = acl_permalink
      end
    end
    ...
    module FinderMethods
      ...
      def find_one(id)
        return super(id) if id.unacl_permalink?
        where(@klass.acl_permalink_config.query_field => id).first or
        with_old_acl_permalink(id) {|x| find_one_without_acl_permalink(x)} or
        find_one_without_acl_permalink(id)
      end

      # Search for a record in the slugs table using the specified slug.
      def exists?(id = false)
        return super if id.unacl_permalink?
        exists_without_acl_permalink?(@klass.acl_permalink_config.query_field => id) or
        with_old_acl_permalink(id) {|x| exists_without_acl_permalink?(x)} or
        exists_without_acl_permalink?(id)
      end

      private

      # Accepts a slug, and yields a corresponding sluggable_id into the block.
      def with_old_acl_permalink(slug, &block)
    ...
    # This module overrides {AclPermalink::SlugGenerator#conflicts} to consider
    ...
    module SlugGenerator
      ...
      def conflicts
        sluggable_class = acl_permalink_config.model_class.base_class
