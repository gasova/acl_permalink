  ...
  module Globalize
    ...
    def self.included(model_class)
      model_class.instance_eval do
        ...
        unless respond_to?('translated_attribute_names') || translated_attribute_names.exclude?(acl_permalink_config.query_field.to_sym)
          puts "\n[AclPermalink] You need to translate '#{acl_permalink_config.query_field}' field with Globalize (add 'translates :#{acl_permalink_config.query_field}' in your model '#{self.class.name}')\n\n"
        end
      end
    end

    module Model
      def set_acl_permalink(text, locale)
        I18n.with_locale(locale || I18n.locale) do
          set_slug(normalize_acl_permalink(text))
        end
      end
    end
    ...
    module FinderMethods
      ...
      def find_one(id)
        ...
        found = where(@klass.acl_permalink_config.query_field => id).first
        found = includes(:translations).
                where(translation_class.arel_table[:locale].in([I18n.locale, I18n.default_locale])).
                where(translation_class.arel_table[@klass.acl_permalink_config.query_field].eq(id)).first if found.nil?
