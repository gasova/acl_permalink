...
class Novelist < ActiveRecord::Base
  extend AclPermalink
  acl_permalink :name, :use => :slugged, :sequence_separator => '_'
...
class SlugGeneratorTest < MiniTest::Unit::TestCase
  ...
  test "should quote column names" do
    model_class = Class.new(ActiveRecord::Base) do
      ...
      self.table_name = "journalists"
      extend AclPermalink
      acl_permalink :name, :use => :slugged, :slug_column => "strange name"
    end
...
class SlugSeparatorTest < MiniTest::Unit::TestCase
  ...
  class Journalist < ActiveRecord::Base
    extend AclPermalink
    acl_permalink :name, :use => :slugged, :sequence_separator => ":"
  end
  ...
  test "should detect when a sequenced slug has changed" do
    with_instance_of model_class do |record|
      record2 = model_class.create! :name => record.name
      assert !record2.should_generate_new_acl_permalink?
  ...
  test "should detect when a sequenced slug has changed when name ends in number and using single dash" do
    ...
    transaction do
      record1 = model_class.create! :name => "Peugeuot 206"
      assert !record1.should_generate_new_acl_permalink?
      record1.save!
      assert !record1.should_generate_new_acl_permalink?
...
class DefaultScopeTest < MiniTest::Unit::TestCase
  ...
  test "acl_permalink should correctly sequence a default_scoped ordered table" do
    transaction do
      3.times { assert Journalist.create :name => "a", :active => true }
    end
  end

  test "acl_permalink should correctly sequence a default_scoped scoped table" do
    transaction do
  ...
  test "should increment the slug sequence for duplicate friendly ids beyond 10" do
    with_instance_of model_class do |record|
      (2..12).each do |i|
        ...
        assert r.acl_permalink.match(/#{i}\z/)
      end
    end
  end
end
...
# https://github.com/norman/acl_permalink/issues/148
