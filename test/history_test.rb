...
class HistoryTest < MiniTest::Unit::TestCase
  ...
  test "should not create new slug record if acl_permalink is not changed" do
    with_instance_of(model_class) do |record|
      record.active = true
      record.save!
      assert_equal 1, AclPermalink::Slug.count
    end
  end

  test "should create new slug record when acl_permalink changes" do
    with_instance_of(model_class) do |record|
      record.name = record.name + "b"
      record.save!
      assert_equal 2, AclPermalink::Slug.count
    end
  end

  test "should be findable by old slugs" do
    with_instance_of(model_class) do |record|
      old_acl_permalink = record.acl_permalink
      record.name = record.name + "b"
      record.save!
      begin
        assert model_class.find(old_acl_permalink)
        assert model_class.exists?(old_acl_permalink), "should exist? by old id"
  ...
  test "should not be read only when found by old slug" do
    with_instance_of(model_class) do |record|
      ...
      record.name = record.name + "b"
      record.save!
      assert !model_class.find(old_acl_permalink).readonly?
    end
  end
  ...
  test "should raise error if used with scoped" do
    ...
    assert_raises RuntimeError do
      model_class.acl_permalink :name, :use => [:history, :scoped]
    end
  end
  ...
  test 'should name table according to prefix and suffix' do
    transaction do
      begin
        prefix = "prefix_"
        without_prefix = AclPermalink::Slug.table_name
        ActiveRecord::Base.table_name_prefix = prefix
        AclPermalink::Slug.reset_table_name
        assert_equal prefix + without_prefix, AclPermalink::Slug.table_name
      ensure
        ActiveRecord::Base.table_name_prefix = ""
        AclPermalink::Slug.table_name = without_prefix
      end
    end
  end
end
